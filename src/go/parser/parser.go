...
func (p *parser) parseNilSafeSelector(x ast.Expr) ast.Expr {
	if p.trace {
		defer un(trace(p, "NilSafeSelector"))
	}

	sel := p.parseIdent()

	return &ast.SelectorExpr{X: x, Sel: sel, NilSafe: true}
}
	...
	for n = 1; ; n++ {
		...
		case token.QUESTION_PERIOD:
			...
			if p.tok == token.IDENT {
				...
				sel := &ast.Ident{NamePos: pos, Name: "_"}
				x = &ast.SelectorExpr{X: x, Sel: sel, NilSafe: true}
			}
		case token.LBRACK:
