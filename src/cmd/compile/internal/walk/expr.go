...
func walkDot(n *ir.SelectorExpr, init *ir.Nodes) ir.Node {
	...
	n.X = walkExpr(n.X, init)
	if n.NilSafe {
		return walkNilSafeDot(n, init)
	}
	return n
}

// walkNilSafeDot lowers a nil-safe selector X?.Sel (Kotlin/JS-style) into:
//
//	tmp := zero(ResultType)
//	if X != nil {
//	    tmp = X.Sel
//	}
//	// tmp used in place of the original expression
//
// X has already been walked (walkDot walks n.X before calling this), so
// nested chains like a?.b?.c compose correctly bottom-up: if a?.b was
// itself nil-safe, n.X here is already the lowered temp holding either
// a.b's value or b's zero value, and this guard handles the next link.
func walkNilSafeDot(n *ir.SelectorExpr, init *ir.Nodes) ir.Node {
	xt := n.X.Type()
	if !xt.IsPtr() && !xt.IsInterface() {
		base.ErrorfAt(n.Pos(), 0, "invalid operation: %v (?. requires pointer or interface, found %v)", n, xt)
		n.NilSafe = false
		return n
	}
	...
	tmp := typecheck.TempAt(pos, ir.CurFunc, resType)
	init.Append(ir.NewAssignStmt(pos, tmp, ir.NewZero(pos, resType)))

	cond := typecheck.Expr(ir.NewBinaryExpr(pos, ir.ONE, n.X, typecheck.NodNil()))
	nif := ir.NewIfStmt(pos, cond, nil, nil)
	nif.Body = []ir.Node{ir.NewAssignStmt(pos, tmp, n)}
	init.Append(nif)
