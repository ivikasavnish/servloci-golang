...
func (p *parser) fileOrNil() *File {
	...
	for p.tok != _EOF {
		...
		case _At:
			// Collect decorators for next function
			decorators := p.decoratorList()
			// Skip any semicolons after decorators
			for p.tok == _Semi {
				p.next()
			}
			if p.tok == _Func {
...
// decoratorList parses one or more decorators (@name or @name(args))
func (p *parser) decoratorList() []*Decorator {
	...
	for p.tok == _At {
		...
		if p.tok == _Lparen {
			// Create a CallExpr for @decorator(args)
			call := new(CallExpr)
			call.pos = d.Name.Pos()
			call.Fun = d.Name
			p.next() // consume (
			call.ArgList, call.HasDots = p.argList()
			d.Call = call
		}
		
		decorators = append(decorators, d)
		
		// Skip semicolons/newlines between decorators
		for p.tok == _Semi {
			p.next()
		}
		
		// Continue if we find another @, otherwise break
		if p.tok != _At {
			break
		}
	}
